...
class Day18 : Puzzle
{
    ...
    public override void Part1()
    {
        RunSearch();
        return;
        List<char> pois = locations.Keys.Where(c => char.IsLower(c) || c == '@').ToList();
        ...
        allPaths = allPaths.Where(p =>
        {
            if (allPaths.Any(p2 =>
            {
                return p.hasPoint(p2.from) && p.hasPoint(p2.to) && p.neededKeys.SequenceEqual(p2.neededKeys) &&
        ...
        allPaths = allPaths.SelectMany(f =>
        {
            return allPaths.Where(p => (p.hasPoint(f.to)) && !p.hasPoint(f.from) && !p.via.Contains(f.from))
        ...
        List<(char[] collected, int cost)> stack = new List<(char[] collected, int cost)>
        {
            (new char[]{'@'}, 0)
        };
        ...
        while (stack.Count > 0)
        {
            (char[] collected, int cost) = stack[^1];
            stack.RemoveAt(stack.Count - 1);

            if (cost >= cheapestRoute) continue;
            var key = collected.OrderBy(a => a).Append(collected[^1]).Aggregate("", (a, b) => a + b);
            if (seenPath.ContainsKey(key) && seenPath[key] <= cost)
            {
                seenUse++; continue;
            }
            seenPath[key] = cost;
            // if (seenPath.Count % 10000 == 0) Console.WriteLine($"Stack {stack.Count} Seen {seenPath.Count} {seenUse} {key}");

            // if (stack.Count<stackLen)
            // {
            //     stackLen = stack.Count;
            //     Console.WriteLine($"Short stack {stack.Count} {collected.Length}");
            // } else if (stack.Count > stackLen*2) stackLen = 45;


            // var key = collected.OrderBy(a=>a).Aggregate("", (a,b)=>a+b);
            // if (seen.ContainsKey(key) && seen[key] <= cost) continue;
            // seen[key] = cost;


            if (collected.Length == pois.Count)
            {
                Console.WriteLine("Found a new winner! " + cost + " " + collected.Aggregate("", (a, b) => a + b));
                cheapestRoute = cost;
            }
            var current = collected[^1];
            var toVisit = allPaths.Where(path =>
                cost + path.cost < cheapestRoute &&
                path.neededKeys.All(collected.Contains) &&
                path.hasPoint(current) &&
                !collected.Contains(path.otherPoint(current))
            ).Select(path => (collected.Union(path.via).Append(path.otherPoint(current)).ToArray(), cost + path.cost));
        ...
        // Console.WriteLine(seen[pois.ToArray()]);
    }
